(() => {
  ...
  function renderBody(text) {
    ...
    for (let i = 0; i < lines.length;) {
      ...
      if (line.startsWith("```")) {
        ...
        const paragraph = [];
        while (i < lines.length && lines[i].trim() && !lines[i].startsWith("|") && !lines[i].startsWith("```")) paragraph.push(lines[i++]);
  ...
  function render() {
    ...
    const visible = sections.filter((section) => (section.title + " " + section.text).toLowerCase().includes(query));
    const container = byId("technicalSections");
    container.replaceChildren();
    visible.forEach((section) => {
      const detail = node("details", undefined, "technical-section");
      detail.open = Boolean(query);
      detail.append(node("summary", section.title), renderBody(section.text));
