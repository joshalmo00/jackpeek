...
| NAS-primary encrypted cache | Successful upload keeps an encrypted local cache copy for 7 days; cleanup runs on sync/save while the app is open | NAS copy remains; local cache cleanup does not affect archive/backups |
| NAS-primary pending upload | Retained locally until upload succeeds; not deleted only because it is old | Corrupt/unreadable records retained for investigation |
...
Warnings use remaining hours for uploaded local cache records, with 24/12-hour defaults, and are written to audit and exposed by the app. The Capture screen NAS health control also shows repository connectivity, retained local cache, pending uploads and cache records close to cleanup, with a force-upload action for pending logs. No guaranteed notification while closed or with long intervals. Unreadable cache may outlive expiry. Filesystem delete does not guarantee forensic erasure or deletion from backups.
